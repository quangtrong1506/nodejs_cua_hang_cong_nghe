...
import { Admin, USERS } from '../../../config/common.mjs';
import paginateRequest from './PaginateRequest.js';

const validationsStoreOrUpdateAdmin = [
    ...
    body('role')
        .isIn(Object.values(Admin.level))
        .withMessage('Giá trị đã chọn trong trường phân quyền không hợp lệ.'),
    body('email')
    ...
    body('password').custom(async (phoneValue) => {
        if (typeof phoneValue !== 'string') {
            throw new Error('Mật khẩu phải là kiểu chuỗi');
        }

        if (phoneValue.length > 30) {
            throw new Error('Mật khẩu không được lớn hơn 30 ký tự');
        }
...
const validationsIndexUser = [
    ...
    query('level')
        .isIn([...Object.values(USERS.level), null])
        .withMessage('Giá trị đã chọn trong trường phân quyền không hợp lệ.'),
];

export const validateIndexUser = baseRequest([
    ...validationsIndexUser,
    ...paginateRequest,
]);
export const validateStoreOrUpdateAdmin = baseRequest(
    validationsStoreOrUpdateAdmin
);
